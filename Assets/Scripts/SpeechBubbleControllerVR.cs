...
public class SpeechBubbleControllerVR : MonoBehaviour, IPointerClickHandler
{
    ...
    public Vector2 debugForcedSize = new Vector2(80f, 80f);
        ...
        public bool useIconWorldOverride = true;
        // Valores por defecto aplicados según tu petición (TransformWorldPlacementJSON)
        public Vector3 iconWorldPosition = new Vector3(-0.36489933729171755f, 2.0447843074798586f, 1.5759018659591675f);
        public Vector3 iconWorldEuler = Vector3.zero;
        public Vector3 iconWorldScale = new Vector3(0.0034175794571638109f, 0.0034175794571638109f, 0.0034175794571638109f);
    [Tooltip("Si está activo, puede pasar un quaternion (x,y,z,w) para la rotación del icon override")]
    public bool useIconWorldQuaternion = true;
    public Vector4 iconWorldQuaternion = new Vector4(-0.11465958505868912f, -0.064142607152462f, 0.006061267573386431f, 0.9913134574890137f);

    [Header("Canvas overlay placement (opcional)")]
    [Tooltip("Si está activo, el canvas usado para el icono se colocará exactamente en la posición/rotación/escala world especificada. Útil para forzar el RectTransform del canvas al que te refieres.")]
    public bool useCanvasWorldOverride = false;
    public Vector3 canvasWorldPosition = Vector3.zero;
    public Vector3 canvasWorldEuler = Vector3.zero;
    public Vector3 canvasWorldScale = Vector3.one;
    [Tooltip("Si está activo, puede pasar un quaternion (x,y,z,w) para la rotación del canvas override")]
    public bool useCanvasWorldQuaternion = false;
    public Vector4 canvasWorldQuaternion = new Vector4(0f,0f,0f,1f);
    ...
    [Tooltip("Offset local respecto al RectTransform parent (ej: Vector3(-0.0469999984,-0.182999998,-0.0659999996))")]
    public Vector3 iconLocalOffset = new Vector3(-0.0469999984f, -0.182999998f, -0.0659999996f);
    ...
    [Header("Manual Mute Canvas (opcional)")]
    [Tooltip("Si tienes un Canvas/manual creado con dos Images (mute/unmute), actívalo y asigna aquí para controlarlo con el botón del mando izquierdo.")]
    public bool useManualMuteCanvas = false;
    public GameObject manualMuteCanvas;
    public Image manualMuteImageMuted;
    public Image manualMuteImageUnmuted;
    [Tooltip("Botón OVR a usar para alternar (por defecto X izquierdo). Puedes cambiar si lo necesitas.")]
    public OVRInput.Button manualToggleButton = OVRInput.Button.Two; // Two suele mapear a Y/ B en algunas versiones; configurable
   ...
   private void ApplyScroll()
    ...
    private void Start()
    {
        ...
        if (muteIcon != null)
        {
            ...
            // Parentear al canvas overlay pero mantener una configuración predecible
            muteIcon.rectTransform.SetParent(canvasHolder.transform, false);
            // Asegurarnos propiedades visuales básicas
            Image imgComp = muteIcon.GetComponent<Image>();
            if (imgComp != null)
            {
            ...
            // Forzar que el canvas del icono tenga overrideSorting alto
            Canvas icCanvas = muteIcon.canvas;
            if (icCanvas != null)
            {
                icCanvas.overrideSorting = true;
                icCanvas.sortingOrder = 32767;
            }
            // estado inicial
            isMuted = false;
            UpdateMuteSprite();
            ...
            // Si el usuario pidió override world del icon, aplicarlo inmediatamente
            if (useIconWorldOverride)
            {
                // Aplicar transform world exacta al icono
                ShowMuteIcon(true, isMuted);
            }
        ...
        // Inicializar canvas/manual mute si el usuario lo configuró
        if (useManualMuteCanvas)
        {
            isMuted = false;
            if (manualMuteCanvas != null)
                manualMuteCanvas.SetActive(true);
            if (manualMuteImageMuted != null)
                manualMuteImageMuted.gameObject.SetActive(false);
            if (manualMuteImageUnmuted != null)
                manualMuteImageUnmuted.gameObject.SetActive(true);
        }
    }

    // estado de silenciado local
    private bool isMuted = false;

    // Llamar desde un botón UI (OnClick) para alternar mute
    public void ToggleMute()
    {
        SetMuted(!isMuted);
    }

    // Toggle específico para el canvas/manual creado por el usuario
    public void ToggleMuteManual()
    {
        // Alternar estado y sólo activar/desactivar las dos imágenes dentro de ManualMuteCanvas
        isMuted = !isMuted;

        if (useManualMuteCanvas)
        {
            if (manualMuteImageMuted != null)
                manualMuteImageMuted.gameObject.SetActive(isMuted);
            if (manualMuteImageUnmuted != null)
                manualMuteImageUnmuted.gameObject.SetActive(!isMuted);
        }
        else
        {
            // comportamiento por defecto si no se usa el canvas manual
            SetMuted(isMuted);
        }
    }

    // Establecer el estado de mute y actualizar sprite/icono
    ...
    private void UpdateMuteSprite()
    {
        ...
        else
        {
            if (spriteUnmuted != null)
            {
                img.sprite = spriteUnmuted;
            }
        }

        // Forzar tipo y tamaño
        img.type = Image.Type.Simple;
        img.preserveAspect = true;
        // Ajustar tamaño del rect según el sprite nativo si queremos que coincida
        try
        {
            img.SetNativeSize();
            // Luego limitar al tamaño configurado
            RectTransform rt = img.rectTransform;
            if (rt != null)
            {
                rt.sizeDelta = muteIconSize;
            }
        }
        catch { }
    }
    ...
    private GameObject EnsureMuteIconCanvas(RectTransform parent)
    {
        ...
        // SortingOrder muy alto para garantizar que siempre esté por encima
        canvas.sortingOrder = 32767;
        // Si hay cámara principal, ayudar al canvas world-space a usarla
        if (Camera.main != null)
            canvas.worldCamera = Camera.main;

        // Ajustar rect transform para coincidir con viewport (tamaño y transform en world-space)
        RectTransform cr = canvasGO.GetComponent<RectTransform>();
        cr.anchorMin = new Vector2(0f, 0f);
        cr.anchorMax = new Vector2(1f, 1f);
        // Por defecto stretch al parent y sizeDelta = 0 para que el rect siga el tamaño del viewport
        cr.sizeDelta = Vector2.zero;
        cr.anchoredPosition = Vector2.zero;
        // Alinearlo con el viewport en posición/rotación por defecto
        cr.position = parent.position;
        cr.rotation = parent.rotation;
        cr.localScale = Vector3.one;

        // Si el desarrollador quiere forzar una transform world explícita para el canvas,
        // aplicarla aquí (esto permite poner exactamente el RectTransform que mostraste)
        if (useCanvasWorldOverride)
        {
            // Aplicar posición world solicitada
            canvasGO.transform.position = canvasWorldPosition;

            // Aplicar rotación: preferir quaternion si se pidió
            if (useCanvasWorldQuaternion)
            {
                Quaternion q = new Quaternion(canvasWorldQuaternion.x, canvasWorldQuaternion.y, canvasWorldQuaternion.z, canvasWorldQuaternion.w);
                canvasGO.transform.rotation = q;
            }
            else
            {
                canvasGO.transform.rotation = Quaternion.Euler(canvasWorldEuler);
            }

            // Aplicar escala world directamente
            canvasGO.transform.localScale = canvasWorldScale;

            // Cuando forzamos la transform world, dejamos el rect transform con anchors stretch y sizeDelta=0
            // y además ajustamos su position/rotation/scale para que coincida exactamente
            RectTransform forced = canvasGO.GetComponent<RectTransform>();
            forced.position = canvasGO.transform.position;
            forced.rotation = canvasGO.transform.rotation;
            forced.localScale = canvasGO.transform.localScale;
            forced.sizeDelta = Vector2.zero;
        }
...
private IEnumerator ThinkingLoop()
{
    ...
    iconRect.pivot = new Vector2(1f, 0f);
        ...
        // Si el usuario quiere un placement world-space explícito, aplicarlo (usar quaternion si está disponible)
        if (useIconWorldOverride)
        {
            // Parentear al canvas overlay conservando la posición/rotación/escala world
            // y aplicar la transform directamente sobre el RectTransform del Image para que coincida exactamente.
            iconRect.SetParent(canvasHolder.transform, worldPositionStays: true);

            // Aplicar posición world exacta sobre el RectTransform
            iconRect.position = iconWorldPosition;

            // Aplicar rotación: quaternion preferido
            if (useIconWorldQuaternion)
            {
                Quaternion q = new Quaternion(iconWorldQuaternion.x, iconWorldQuaternion.y, iconWorldQuaternion.z, iconWorldQuaternion.w);
                iconRect.rotation = q;
            }
            else
            {
                iconRect.rotation = Quaternion.Euler(iconWorldEuler);
            }

            // Para asegurar que la escala WORLD del icono sea exactamente iconWorldScale,
            // debemos asignar localScale = desiredWorldScale / parent.lossyScale
            Vector3 desiredWorldScale = iconWorldScale;
            Vector3 parentLossy = Vector3.one;
            if (muteIcon.transform.parent != null)
                parentLossy = muteIcon.transform.parent.lossyScale;

            Vector3 safeParentLossy = new Vector3(
                parentLossy.x == 0f ? 1f : parentLossy.x,
                parentLossy.y == 0f ? 1f : parentLossy.y,
                parentLossy.z == 0f ? 1f : parentLossy.z
            );

            Vector3 computedLocalScale = new Vector3(
            ...
            computedLocalScale.x = float.IsNaN(computedLocalScale.x) || float.IsInfinity(computedLocalScale.x) ? desiredWorldScale.x : computedLocalScale.x;
            computedLocalScale.y = float.IsNaN(computedLocalScale.y) || float.IsInfinity(computedLocalScale.y) ? desiredWorldScale.y : computedLocalScale.y;
            computedLocalScale.z = float.IsNaN(computedLocalScale.z) || float.IsInfinity(computedLocalScale.z) ? desiredWorldScale.z : computedLocalScale.z;

            // Aplicar escala local calculada sobre el RectTransform para obtener la escala WORLD deseada
            iconRect.localScale = computedLocalScale;

            // Asegurar que la Image use material/shader UI por defecto y esté visible
            Image img = muteIcon.GetComponent<Image>();
            if (img != null)
            {
                img.raycastTarget = false;
                ...
                img.type = Image.Type.Simple;
                img.preserveAspect = true;
                // Intentar asignar tamaño nativo para apariencia correcta
                try { img.SetNativeSize(); } catch { }
            }

            // Ajustar sizeDelta del rect transform para que el rect tenga el tamaño esperado en unidades locales
            RectTransform rt = muteIcon.rectTransform;
            if (rt != null)
            {
                // Alinear anchors y pivot al centro para que la posición world aplicada no se desplace
            ...
            // Forzar visible y render por encima
            muteIcon.gameObject.SetActive(true);
        ...
        if (useLocalOffset)
        {
            // Queremos que el icono esté en un canvas overlay (MuteIconCanvas) pero posicionado
            // en world-space relativo al RectTransform 'viewport'. Para ello:
            // 1) calcular worldPos = viewport.TransformPoint(iconLocalOffset)
            // 2) parentear el icono al canvasHolder con worldPositionStays = true
            // 3) asignar position/rotation/scale para que quede delante del chat y por encima en render

            GameObject overlay = canvasHolder; // EnsureMuteIconCanvas(viewport) ya fue llamado arriba

            // Calcular posición world desde el offset local del viewport
            Vector3 worldPos = viewport.TransformPoint(iconLocalOffset);

            // Parentear al canvas overlay conservando world pos y aplicar la posición calculada
            iconRect.SetParent(overlay.transform, worldPositionStays: true);
            // Colocar ligeramente por delante del chat para evitar z-fighting/ocultamiento
            float frontOffset = 0.005f; // en unidades locales del mundo (ajusta si es necesario)
            muteIcon.transform.position = worldPos + (viewport.forward * frontOffset);
            // Mantener la orientación del viewport para que el icono 'mire' igual que la burbuja
            muteIcon.transform.rotation = viewport.rotation;

            // Ajustar escala independientemente para evitar que la escala del viewport lo aplaste
            muteIcon.transform.localScale = Vector3.one * Mathf.Max(0.0001f, muteIconWorldScale);

            // Tamaño en píxeles razonable
            iconRect.sizeDelta = muteIconSize;

            // Forzar que el icono esté visible/oculto según parámetro
            muteIcon.gameObject.SetActive(show);

            // Asegurar que se renderice por encima
            muteIcon.transform.SetAsLastSibling();
